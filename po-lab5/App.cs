...
class App
{
    public static void Main(string[] args)
    {
        ...
        // var limitedHex = hex.GetLimitedHex(2);
        ...
        // Console.WriteLine(board.GetPieceCount(ChessPiece.Empty));
...
class Exercise4
{
    ...
    public Exercise4()
    {
        for (int i = 0; i < 8; i++)
            for (int j = 0; j < 8; j++)
                _board[i, j] = (ChessPiece.Empty, i % 2 != j % 2 ? ChessColor.White : ChessColor.Black);
    }
    ...
    public (ChessPiece, ChessColor) this[string coord]
    {
        ...
        set
        {
            if(value.Item1 == ChessPiece.Queen && GetPieceCount(ChessPiece.Queen) > 1)
                throw new InvalidChessPieceCount();

            if(value.Item1 == ChessPiece.King && GetPieceCount(ChessPiece.King) > 1)
                throw new InvalidChessPieceCount();

            if(value.Item1 == ChessPiece.Rook && GetPieceCount(ChessPiece.Rook) > 2)
                throw new InvalidChessPieceCount();

            if(value.Item1 == ChessPiece.Bishop && GetPieceCount(ChessPiece.Bishop) > 2)
                throw new InvalidChessPieceCount();

            if(value.Item1 == ChessPiece.Knight && GetPieceCount(ChessPiece.Knight) > 2)
                throw new InvalidChessPieceCount();

            if(value.Item1 == ChessPiece.Pawn && GetPieceCount(ChessPiece.Pawn) > 8)
                throw new InvalidChessPieceCount();
    ...
    public int GetPieceCount(ChessPiece piece)
    {
        int count = 0;
        for (int i = 0; i < 8; i++)
        {
